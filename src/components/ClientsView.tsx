    ...
    <div className="space-y-4 pb-20">
          ...
          {filteredClientes.map((cliente: Cliente) => {
            ...
            return (
              <div
                ...
                className="bg-white shadow-sm hover:bg-white border border-slate-200 rounded-2xl p-4 cursor-pointer transition-all hover:border-emerald-500 shadow-sm relative group"
              >
                <div className="flex items-start justify-between gap-2 mb-2">
                  ...
                  <div className="flex items-center gap-1">
                    ...
                    <button
                      ...
                      className="p-1.5 hover:bg-red-500/20 text-slate-400 hover:text-red-600 rounded-lg transition-colors"
                      title="Eliminar"
                    >
                ...
                <div className="flex items-center justify-between pt-2 border-t border-slate-100 text-sm">
                  ...
                  {totalDeuda > 0 ? (
                    <span className="font-bold text-amber-600 text-sm bg-amber-50 px-2 py-0.5 rounded-md border border-amber-200">
                      Deuda: {formatCurrency(totalDeuda)}
                    </span>
                  ) : (
                    <span className="text-emerald-600 text-xs font-semibold">Al día</span>
                  )}
                </div>
              </div>
            );
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-50 bg-black/40 backdrop-blur-sm flex items-center justify-center p-4 overflow-y-auto">
          <div className="bg-white border border-slate-200 rounded-2xl max-w-md w-full p-5 space-y-4 shadow-2xl animate-in fade-in zoom-in-95 duration-200">
            ...
            <form onSubmit={handleSubmit} className="space-y-3">
              ...
              <div>
                <label className="block text-sm font-semibold text-slate-600 mb-1">Notas</label>
                <textarea
                  rows={2}
              ...
              <div className="flex items-center justify-end gap-2 pt-2 border-t border-slate-200">
                ...
                <button
                  type="submit"
                  className="px-4 py-2 rounded-xl text-sm font-semibold bg-emerald-600 hover:bg-emerald-500 text-white shadow-md shadow-emerald-600/20"
                >
      ...
      {selectedCliente && (
        <div className="fixed inset-0 z-50 bg-black/40 backdrop-blur-sm flex items-center justify-center p-4 overflow-y-auto">
          <div className="bg-white border border-slate-200 rounded-2xl max-w-lg w-full p-5 space-y-4 shadow-2xl max-h-[90vh] flex flex-col">
            ...
            <div className="space-y-4 overflow-y-auto pr-1 flex-1">
              ...
              <div>
                <h4 className="text-sm font-bold text-slate-600 uppercase tracking-wider mb-2 flex items-center gap-1.5">
                  <FileText className="w-4 h-4 text-amber-600" /> Facturas Emitidas
                </h4>
                  ...
                  <div className="space-y-1.5">
                    {state.facturas
                      ...
                      .map((fac) => (
                        <div
                          key={fac.id}
                          className="bg-white shadow-sm p-2.5 rounded-xl flex items-center justify-between text-sm border border-slate-200"
                        >
                          <div>
              ...
              <div>
                ...
                {state.prestamos.filter((p) => p.cliente_id === selectedCliente.id).length === 0 ? (
                  <p className="text-sm text-slate-400 italic">Sin préstamos activos.</p>
                ) : (
                  <div className="space-y-1.5">
                    {state.prestamos
            ...
            <div className="pt-2 border-t border-slate-200 text-right">
              <button
                onClick={() => setSelectedCliente(null)}
                className="px-4 py-2 bg-white hover:bg-slate-100 text-slate-700 text-sm font-semibold rounded-xl"
              >
                Cerrar Ficha
              </button>
            </div>
