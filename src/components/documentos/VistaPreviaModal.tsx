...
import { construirUrl } from '../../utils/whatsapp';
...
interface VistaPreviaModalProps {
  titulo: string;
  subtitulo?: string;
  /** Nombre del archivo sin extensión; se le añade el formato si es rollo. */
  archivoBase: string;
  /** Título que ve el usuario en la hoja de compartir del sistema. */
  tituloCompartir: string;
  /** Mensaje que acompaña al PDF al compartirlo. */
  textoWhatsapp: string;
  telefono?: string | null;
  onClose: () => void;
  children: (contexto: ContextoDocumento) => React.ReactNode;
}

/**
 * Vista previa de cualquier documento imprimible, con su selector de
 * formato y las acciones de descargar, imprimir y compartir.
 *
 * El documento en sí lo pone quien la usa: así la factura, el comprobante
 * de préstamo y el recibo de abono comparten exactamente el mismo
 * comportamiento —incluidos los tres formatos de papel— sin repetir la
 * barra de botones ni la lógica de generación en cada sitio.
 */
export const VistaPreviaModal: React.FC<VistaPreviaModalProps> = ({
  titulo,
  subtitulo = 'Listo para imprimir o descargar en PDF.',
  archivoBase,
  tituloCompartir,
  textoWhatsapp,
  telefono,
  ...
  const documentElementId = `doc-${idBase}-${formato}`;
  const filename = `${archivoBase}${
    definicion.termico ? `-${definicion.etiqueta.replace(' ', '')}` : ''
  }.pdf`;
   ...
   * compartir del sistema y es el propio WhatsApp quien muestra la lista
   * de contactos. En escritorio, donde no se pueden compartir archivos, se
  ...
  const compartir = async () => {
    ...
    try {
      ...
      const resultado = await compartirArchivo(archivo, tituloCompartir, textoWhatsapp);

      if (resultado === 'sin-soporte') {
        descargarArchivo(archivo);
        window.open(construirUrl(textoWhatsapp, telefono), '_blank', 'noopener,noreferrer');
        avisarInfo(
  ...
  return (
    <div className="fixed inset-0 z-[60] bg-slate-900/50 backdrop-blur-md flex items-start sm:items-center justify-center p-3 overflow-y-auto">
      <div className="bg-white border border-slate-200 rounded-2xl max-w-3xl w-full p-4 space-y-3 shadow-2xl max-h-[95vh] flex flex-col my-4">
        ...
        {/* El documento tiene un ancho fijo y no depende de puntos de corte
            responsivos: así el PDF y la impresión salen idénticos desde un
            móvil o desde un ordenador. En pantallas estrechas la vista
            previa se desplaza en horizontal. */}
        <div className="overflow-auto pr-1 flex-1 bg-slate-100 p-2 sm:p-4 rounded-xl border border-slate-200">
          {children({
            id: documentElementId,
            formato,
            termico: definicion.termico,
          })}
        </div>
      </div>
    </div>
