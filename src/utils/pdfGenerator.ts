...
function opciones(filename: string) {
  return {
...
export async function generatePdfFromElement(elementId: string, filename: string): Promise<void> {
  ...
  await conColoresNormalizados(element, () =>
    html2pdf().set(opciones(filename)).from(element).save()
  );
}

/** El mismo PDF de la descarga, en memoria, para poder compartirlo. */
export async function generarPdfBlob(elementId: string, filename: string): Promise<Blob> {
  const element = elementoODescartar(elementId, 'exportar');
  const salida = await conColoresNormalizados(element, () =>
    html2pdf().set(opciones(filename)).from(element).outputPdf('blob')
  );
  return salida instanceof Blob ? salida : new Blob([salida], { type: 'application/pdf' });
}

export async function generarPdfFile(elementId: string, filename: string): Promise<File> {
  const blob = await generarPdfBlob(elementId, filename);
